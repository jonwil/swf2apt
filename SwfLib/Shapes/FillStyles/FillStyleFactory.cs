...
namespace SwfLib.Shapes.FillStyles {
    public class FillStyleFactory {
        ...
        public FillStyleRGBA CreateMorphRGBA(FillStyleType type)
        {
            switch (type)
            {
                case FillStyleType.SolidColor:
                    return new SolidMorphFillStyleRGBA();
                case FillStyleType.RepeatingBitmap:
                    return new BitmapMorphFillStyleRGBA { Smoothing = true, Mode = BitmapMode.Repeat };
                case FillStyleType.ClippedBitmap:
                    return new BitmapMorphFillStyleRGBA { Smoothing = true, Mode = BitmapMode.Clip };
                case FillStyleType.NonSmoothedRepeatingBitmap:
                    return new BitmapMorphFillStyleRGBA { Smoothing = false, Mode = BitmapMode.Repeat };
                case FillStyleType.NonSmoothedClippedBitmap:
                    return new BitmapMorphFillStyleRGBA { Smoothing = false, Mode = BitmapMode.Clip };
                default:
                    throw new NotSupportedException();
            }
        }
