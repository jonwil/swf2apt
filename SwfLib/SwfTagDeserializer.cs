    ...
    public class SwfTagDeserializer : ISwfTagVisitor<ISwfStreamReader, SwfTagBase> {
        ...
        SwfTagBase ISwfTagVisitor<ISwfStreamReader, SwfTagBase>.Visit(PlaceObject2Tag tag, ISwfStreamReader reader) {
            ...
            if (tag.HasColorTransform) {
                ...
                var transform = tag.ColorTransform;
        ...
        SwfTagBase ISwfTagVisitor<ISwfStreamReader, SwfTagBase>.Visit(ImportAssetsTag tag, ISwfStreamReader reader) {
            tag.URL = reader.ReadString();
            int count = reader.ReadUInt16();
            for (int i = 0; i < count; i++)
            {
                tag.Tags.Add(reader.ReadUInt16());
                tag.Names.Add(reader.ReadString());
            }
            return tag;
        }
        ...
        SwfTagBase ISwfTagVisitor<ISwfStreamReader, SwfTagBase>.Visit(DoActionTag tag, ISwfStreamReader reader) {
            ...
            for (int i = 0; i < tag.ActionRecords.Count; i++)
            {
                tag.ActionRecords[i].SetLabel(i, tag.ActionRecords);
            }
            return tag;
        }
        ...
        SwfTagBase ISwfTagVisitor<ISwfStreamReader, SwfTagBase>.Visit(DefineMorphShapeTag tag, ISwfStreamReader reader) {
            tag.CharacterID = reader.ReadUInt16();
            tag.StartBounds = reader.ReadRect();
            tag.EndBounds = reader.ReadRect();
            reader.ReadUInt32();
            reader.ReadToMorphFillStylesRGBA(tag.StartFillStyles, tag.EndFillStyles);
            reader.ReadToMorphLineStylesRGBA(tag.StartLineStyles, tag.EndLineStyles);
            reader.ReadToShapeRecordsRGBA(tag.StartShapeRecords);
            foreach (var record in tag.StartShapeRecords)
            {
                if (record is StyleChangeShapeRecordRGBA r)
                {
                    var fill0 = r.FillStyle0;
                    var fill1 = r.FillStyle1;
                    r.FillStyle1 = fill0;
                    r.FillStyle0 = fill1;
                }
            }
            reader.ReadToShapeRecordsRGBA(tag.EndShapeRecords);
            foreach (var record in tag.EndShapeRecords)
            {
                if (record is StyleChangeShapeRecordRGBA r)
                {
                    var fill0 = r.FillStyle0;
        ...
        SwfTagBase ISwfTagVisitor<ISwfStreamReader, SwfTagBase>.Visit(DefineButton2Tag tag, ISwfStreamReader reader) {
            ...
            while (actionsOffset != 0 ? (reader.Position - baseOffset) < actionsOffset : !reader.IsEOF) {
                var c = reader.ReadButtonRecordEx();
                if (actionsOffset != 0 ? (reader.Position - baseOffset) < actionsOffset : !reader.IsEOF)
                {
                    tag.Characters.Add(c);
                }
            }
